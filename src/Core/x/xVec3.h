...
struct xVec3
{
	...
	xVec3 cross(const xVec3& c) const WIP { xVec3 v = {}; v.x = y * c.z - c.y * z; v.y = z * c.x - c.z * x; v.z = x * c.y - c.x * y; return v; }
	float32 dot(const xVec3& c) const WIP { return x * c.x + y * c.y + z * c.z; }
	float32 length() const WIP { return xsqrt(length2()); }
	float32 length2() const WIP { return x * x + y * y + z * z; }
	...
	xVec3& set_abs() WIP { x = iabs(x); y = iabs(y); z = iabs(z); return *this; }
	xVec3 get_abs() const WIP { xVec3 v = *this; v.set_abs(); return v; }
